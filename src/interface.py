...
class Interface:
    """
    Класс для управления интерфейсом приложения.
    """
    def __init__(self, bank_service, report_generator):
        """
    ...
    def run(self):
        """
        Запускает основной цикл работы интерфейса.
        """
        try:
            self.logger.info("Запуск интерфейса приложения")
            display_interface()

        except Exception as e:
            self.logger.error(f"Ошибка при работе интерфейса: {e}")
            raise
